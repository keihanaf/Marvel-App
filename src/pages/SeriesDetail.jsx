...
import { FaBook, FaHeart, FaRegHeart, FaUser } from "react-icons/fa";
...
function SeriesDetail({ likedSeries, toggleLike }) {
    const { seriesId } = useParams();
    const { seriesDetails } = useSeries();
    const { seriesDetail, isSeriesDetailLoading, error } = seriesDetails(seriesId);
    const navigate = useNavigate();
    const isLiked = likedSeries.includes(seriesId);
    ...
    const handleToggleLike = () => {
        ...
        const seriesDetails = {
            ...
            thumbnail: seriesDetail.thumbnail,
            title: seriesDetail.title,
        };
        toggleLike(seriesId, seriesDetails, 'series');
    };

    if (isSeriesDetailLoading) {
        return (
        ...
        <div className="h-[100dvh] flex items-center justify-center bg-[#121212] p-2 sm:p-4 md:p-10">
            <div className="bg-gray-900 rounded-lg overflow-hidden shadow-2xl w-full max-w-4xl h-[90dvh] md:h-auto flex flex-col md:flex-row">
                <div className="w-full md:w-1/2 h-[30dvh] md:h-[600px] relative overflow-hidden">
                    <img
                        src={`${seriesDetail.thumbnail.path}.${seriesDetail.thumbnail.extension}`}
                ...
                <div className="w-full md:w-1/2 flex flex-col h-[60dvh] md:h-[600px]">
                    <div className="p-4 pb-2">
                        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white">
                            {removeHashAndAfter(seriesDetail.title)}
                        </h1>
                    </div>
                    <div className="flex-1 overflow-y-auto px-4">
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {seriesDetail.creators.items.map((item, index) => (
                        ...
                        <div className="mt-4 p-4 bg-gray-800 rounded-lg shadow-md">
                            <p className="text-gray-300">
                                ...
                                <span className="font-semibold text-blue-400">
                                    {convertToEnglishDate(seriesDetail.modified)}
                                </span>
                            </p>
                        </div>
                    </div>
...
export default withLike(SeriesDetail);